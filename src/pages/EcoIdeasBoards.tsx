      ...
      <Tabs value={selectedStrategyId} onValueChange={setSelectedStrategyId} className="w-full">
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 lg:grid-cols-7 h-auto p-2 items-stretch">
          {strategies.map((strategy) => {
            ...
            if (strategy.id === '2') {
              strategyName = 'MINIMISING ENERGY CONSUMPTION';
            }
        ...
        {strategies.map((strategy, strategyIndex) => {
          ...
          return (
            <TabsContent key={strategy.id} value={strategy.id} className="mt-6 pt-4">
              <h3 className="text-2xl font-palanquin font-semibold text-app-header mb-4">{strategy.id}. {strategyName}</h3>
              ...
              <div className="relative flex min-h-[400px] p-8 rounded-lg bg-gray-50 overflow-hidden">
                ...
                <div className="w-1/2 pr-8">
                  {strategy.subStrategies.map((subStrategy, subStrategyIndex) => {
                    ...
                    let subStrategyName = subStrategy.name;
                    if (strategy.id === '2') {
                      switch (subStrategy.id) {
                        case '2.1':
                          subStrategyName = 'Minimise energy consumption during pre-production and production';
                          break;
                        case '2.2':
                          subStrategyName = 'Minimise energy consumption during transportation and storage';
                          break;
                        case '2.3':
                          subStrategyName = 'Select the most efficient energy consumption systems during use';
                          break;
                        case '2.4':
                          subStrategyName = 'Enable a variable consumption of energy, to follow demand fluctuations';
                          break;
                        case '2.5':
                          subStrategyName = 'Minimise energy consumption during product development';
                          break;
                        default:
                          break;
                      }
                    ...
                    return (
                      <div key={subStrategy.id} className="mb-6">
                        <h4 className="text-xl font-palanquin font-semibold text-app-header mb-2">
                          {subStrategy.id}. {subStrategyName}
                        </h4>
                        <ul className="list-none space-y-1">
                          {subStrategy.guidelines.map((guideline, guidelineIndex) => {
                            ...
                            if (strategy.id === '2' && subStrategy.id === '2.5') {
                                switch (guidelineIndex) {
                                    case 0:
                                        displayGuidelineName = "Minimise the consumption of stationery goods and their packages";
                                        break;
                                    case 1:
                                        displayGuidelineName = "Engage digital tools in designing, modelling and prototyping";
                                        break;
                                    case 2:
                                        displayGuidelineName = "Engage digital tools for documentation, communication and presentation";
                                        break;
                                    default:
                                        // Fallback to original name if index is unexpected
                                        displayGuidelineName = guideline.name;
                                        break;
                                }
                            }
