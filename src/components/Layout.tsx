    ...
    <div className={cn(
          ...
          <nav className="mt-2 md:mt-0">
            <ul className="flex flex-wrap gap-2 md:gap-4 justify-center md:justify-start">
              {headerNavItems.map((item) => (
                <li key={item.path}>
                  {item.path === '/' ? (
                    // Render "ICS Toolkit" as a clickable title with orange text
                    <Link to={item.path} className="text-orange-500 text-2xl font-palanquin font-semibold hover:text-orange-600 transition-colors px-4 py-2">
                      {item.label}
                    </Link>
                  ) : (
                    // Render other nav items as buttons with orange hover/active states
                    <Button asChild variant="ghost" className={cn(
                      "text-app-header hover:bg-orange-500 hover:text-white font-roboto-condensed",
      ...
      <footer className="relative w-full py-4">
        ...
        <div className="absolute bottom-4 left-4 flex items-center gap-2">
          <div className="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center text-white cursor-pointer hover:bg-orange-600 transition-colors">
            <HelpCircle size={20} />
          </div>
        </div>
